#!/usr/bin/env bash
#
# testcase-example: executing commands and printing annotated output
#
...
# and based on the command return-code signal test.fail / test.pass. A couple of helpers are called
# to annotate the output.
#
...
cij.info "About to execute a command on the test-target"
cij.warn "In case this fails, and error-message will be printed"

if ! cij.cmd "hostname"; then
  cij.err "Something do not work, check the logs."
  test.fail
fi
